...
sub unicode_to_transliterate($)
{
    ...
    while ($text ne '')
    {
        ...
        elsif ($text =~ s/^(.)//o)
        {
             ...
             #Â that there is no useful transliteration of the unicode character
             #Â instead we want to keep it as is.
             #Â This is the case, for example, for @exclamdown, is corresponds
             #Â with x00a1, but unidecode transliterates it to a !, we want
             #Â to avoid that and keep x00a1.
...
#    my $top_name = $Texi2HTML::Config::TOP_HEADING || $element_top_text || $Texi2HTML::THISDOC{'title'} || $Texi2HTML::THISDOC{'shorttitle'} || &$I('Top');
    ...
    my $cline;
    #Â this is true for the state that goes through the document
